...
import { Assignment, Comment, Issue, IssueCategory, IssueStatus, StatusHistory } from '@/types';
...
export default function IssueDetailScreen() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const commentsData = await api.get<Comment[]>(`/api/issues/${id}/comments`, session ?? undefined).catch(() => []);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        {/* Worker updates */}
        <View style={styles.card}>
          <Text style={styles.cardHeading}>Updates from Worker</Text>
          {comments.length === 0 ? (
            <Text style={styles.unassigned}>No updates yet.</Text>
          ) : (
            comments.map(c => {
