...
import { Assignment, Comment, Issue, IssueCategory, Profile } from '@/types';
...
export default function ManagerIssueDetail() {
  ...
  const { profile: manager, session } = useAuth();
  ...
  async function loadData() {
    const issueData = await api.get<Issue>(`/api/issues/${id}`, session!).catch(() => null);
    if (issueData) {
      setIssue(issueData);
      const asgn = issueData.assignment ?? null;
      setAssignment(asgn);
      setSelectedWorkerId(asgn?.worker_id ?? null);
    } else {
      setAssignment(null);
    }
    const commentsData = await api.get<Comment[]>(`/api/issues/${id}/comments`, session!).catch(() => []);
    setComments(commentsData);
  }

  async function loadWorkers() {
    const data = await api.get<Profile[]>('/api/users?role=worker&is_active=true', session!).catch(() => []);
    setWorkers(data);
  }
  ...
  async function confirmAssignment() {
    ...
    try {
      await api.post(`/api/issues/${issue.id}/assign`, { worker_id: selectedWorkerId }, session!);
      await loadData();
  ...
  async function markInProgress() {
    if (!manager || !issue || !session) return;
  ...
  async function closeIssue() {
    ...
    setActionLoading(true);
    try {
      await api.patch(`/api/issues/${issue.id}`, { status: 'resolved' }, session);
      setCloseConfirm(false);
      await loadData();
    } catch (err: any) {
      Alert.alert('Error', err.message ?? 'Failed to close issue');
    } finally {
      setActionLoading(false);
    }
  }
  ...
  async function deleteIssue() {
    ...
    setActionLoading(true);
    try {
      await api.delete(`/api/issues/${issue.id}`, session);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        {/* Worker updates */}
        <View style={styles.card}>
          <Text style={styles.cardHeading}>Worker Updates</Text>
          {comments.length === 0 ? (
            <Text style={styles.noWorkers}>No updates from worker yet.</Text>
          ) : (
            comments.map(c => {
              const name = c.author?.name ?? 'Worker';
              return (
                <View key={c.id} style={styles.commentItem}>
                  <View style={styles.commentHeader}>
                    <View style={styles.commentAvatar}>
                      <Text style={styles.commentInitial}>{name.charAt(0).toUpperCase()}</Text>
                    </View>
                    <View>
...
const styles = StyleSheet.create({
  ...
  commentInitial: { color: colors.text, fontSize: 13, fontWeight: '700' },
  ...
  commentImage: { width: '100%', height: 150, borderRadius: radius.md },
});
