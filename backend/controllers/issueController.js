...
async function getOne(req, res) {
  try {
    ...
    const [issueRes, historyRes, assignmentRes] = await Promise.all([
      ...
      supabase
        .from('assignments')
        .select('*, worker:profiles!worker_id(id,name,email,role,avatar_url,is_active,created_at,updated_at)')
        .eq('issue_id', id)
        .maybeSingle(),
    ]);
...
async function getComments(req, res) {
  try {
    ...
    const { data, error } = await supabase
      .from('comments')
      .select('*, author:profiles!user_id(id,name,email,role,avatar_url,is_active,created_at,updated_at)')
      .eq('issue_id', id)
