...
async function login(req, res) {
  try {
    ...
    console.log('2. Supabase signIn result:', { error: signInError?.message });

    if (signInError || !authData.session) {
      return res.status(401).json({ error: signInError?.message || 'Login failed' });
    }
    ...
    console.log('3. Profile fetch result:', { profile, error: profileError?.message });
    ...
    console.log('4. Sending response:', { role: profile?.role });
